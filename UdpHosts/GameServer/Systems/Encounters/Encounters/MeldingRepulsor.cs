...
{
    private const uint _effectOffline = 2958;
    private const uint _effectOnline = 2957;
    private const uint _effectInitializing = 3285;
    private const uint _effectCollapse = 5157;

    private const uint _maxMeldedCrystite = 4000;
    private const uint _pushbackDurationMs = 1224_000;
        ...
        var uiQuery = new NewUiQuery()
                      {
                          ...
                          Inputs =
                                   [
                                       ...
                                       {
                                           Id = 4,
                                           Value = _maxMeldedCrystite - _meldedCrystite,
                                       }
                                   ]
                      };
    ...
    public void OnDonation(UiQueryResponse response, INetworkPlayer player)
    {
        ...
        _meldedCrystite += (uint)response.Outputs[0].Amount;

        _terminal.Deployable_ObserverView.SinCardFields_11Prop = new SinCardFieldData()
            {
                Type = SinCardFieldData.SincardFieldDataType.Float,
                Float = _maxMeldedCrystite - _meldedCrystite,
            };

        if (_meldedCrystite < _maxMeldedCrystite)
        {
            return;
        }
        ...
        Shard.Abilities.DoRemoveEffect(_terminal, _effectOffline);
        Shard.Abilities.DoApplyEffect(_effectInitializing, _terminal, new Context(Shard, _terminal) { InitTime = Shard.CurrentTime });
    ...
    public override void OnSignal()
    {
        ...
        Shard.Abilities.DoRemoveEffect(_terminal, _effectInitializing);
        Shard.Abilities.DoApplyEffect(_effectOnline, _terminal, new Context(Shard, _terminal) { InitTime = Shard.CurrentTime });

        Shard.EncounterMan.SetRemainingLifetime(this, _pushbackDurationMs);
                ...
                Timer = new Timer()
                        {
                            ...
                            Micro = (Shard.CurrentTimeLong + _pushbackDurationMs) * 1000,
                        },
            };
    }
    ...
    public void OnTimeOut()
    {
        ...
        {
            Shard.Abilities.DoApplyEffect(_effectCollapse, participant.CharacterEntity, new Context(Shard, _repulsor) { InitTime = Shard.CurrentTime });
        }

        Shard.Abilities.DoApplyEffect(9487, _repulsor, new Context(Shard, _repulsor) { InitTime = Shard.CurrentTime });

        Shard.Abilities.DoRemoveEffect(_terminal, _effectOnline);
        Shard.Abilities.DoApplyEffect(_effectOffline, _terminal, new Context(Shard, _terminal) { InitTime = Shard.CurrentTime });
    ...
    {
        float elapsedTime = Math.Min(currentTime - _startTime, 22_000);
